...
def test_create_card(db):
    ...
    new_secret = Secret(name="Murderer", content="Your the murderer")
    db.add(new_secret)
    db.commit()
    ...
    assert new_secret.name == "Murderer"
    ...
    assert schema_secret.name == new_secret.name
    ...
    assert card_dict["name"] == "Murderer"
