...
async def time_out(match_id: UUID, player_id:UUID, db = Depends(get_db)) -> Optional[List[Match_Card_Schema]]:
    try:
        ...
        if fourth_card:
            if (fourth_card.player_id is None and fourth_card.match_id == match_id):
                fourth_card.player_id = player_id
                fourth_card.is_discarded = False
                fourth_card.discarded_at = None
            db.commit()
            db.refresh(fourth_card)
            
            if fourth_card.player_id != player_id:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail=f"La carta no se actualizó y el player_id es {fourth_card.player_id} --- {fourth_card}"
                )
        else:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
