...
def test_get_secrets(client, db_session):
    ...
    response = client.post(
        "/players",
        json={"name": "creador", "avatar": "avatar1", "birthday": "2000-01-02"},
    )
    ...
    response = client.post(
        "/players",
        json={"name": "hernan", "avatar": "avatar1", "birthday": "2000-06-21"},
    )
    assert response.status_code == 201
    new_player = response.json()
    uuid.UUID(new_player["id"])
    response = client.post(
    ...
    db_session.add_all(
        [
            ...
            Secret(
                id=uuid.uuid4(), type="ACCOMPLICE", content="You are the Accomplice!"
            ),
    ...
    match_secret1 = Match_Secret(
        secret_id=secret1.id, match_id=match_id, player_id=owner_id
    )
    match_secret2 = Match_Secret(
        secret_id=secret2.id, match_id=match_id, player_id=owner_id
    )
    match_secret3 = Match_Secret(
        secret_id=secret3.id, match_id=match_id, player_id=owner_id
    )
    match_secret4 = Match_Secret(
        secret_id=secret4.id, match_id=match_id, player_id=owner_id
    )
    match_secret5 = Match_Secret(
        secret_id=secret5.id, match_id=match_id, player_id=owner_id
    )
    match_secret6 = Match_Secret(
        secret_id=secret6.id, match_id=match_id, player_id=owner_id
    )
    db_session.add_all(
        [
            match_secret1,
    ...
    for secret_data in data:
        assert (
            secret_data["id"] != secret_data["secret_id"]
        ), f"id y secret_id no deben ser iguales: {secret_data}"
        assert "match_id" in secret_data, "Falta campo match_id"
        assert secret_data["match_id"] == str(
            match_id
        ), f"match_id incorrecto: {secret_data['match_id']}"


def test_get_secrets_empty_match(client, db_session):
    """Test get secrets empty match.
    ...
    response = client.post(
        "/players",
        json={"name": "creador", "avatar": "avatar1", "birthday": "2000-01-01"},
    )
    owner = response.json()
    response = client.post(
        "/matches",
