...
from app.matches.schemas import MatchIn, MatchOut
...
def test_create_match(db_session):
    ...
    new_player_owner = Player(
        name="Elian", avatar="Mao Zedong", birthday=date(2000, 8, 22)
    )
    db_session.add(new_player_owner)
    db_session.commit()
    db_session.refresh(new_player_owner)
    new_match = Match(
        name="AGE 3", min_players=4, max_players=6, owner_id=new_player_owner.id
    )
