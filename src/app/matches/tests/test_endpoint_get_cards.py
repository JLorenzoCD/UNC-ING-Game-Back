...
def test_get_cards(client, db_session):
    ...
    response = client.post(
        "/players",
        json={"name": "Owner", "avatar": "avatar1", "birthday": "2000-01-01"},
    )
    ...
    response = client.post(
        "/players",
        json={"name": "Jugador Nuevo", "avatar": "avatar2", "birthday": "2000-02-02"},
    )
    assert response.status_code == 201
    ...
    match_card1 = Match_Card(card_id=card1.id, match_id=match_id, player_id=owner_id)
    match_card2 = Match_Card(card_id=card2.id, match_id=match_id, player_id=owner_id)
    match_card3 = Match_Card(card_id=card3.id, match_id=match_id, player_id=owner_id)
    match_card4 = Match_Card(
        card_id=card4.id, match_id=match_id, player_id=new_player_id
    )
    match_card5 = Match_Card(
        card_id=card5.id, match_id=match_id, player_id=new_player_id
    )
    db_session.add_all(
        [match_card1, match_card2, match_card3, match_card4, match_card5]
    )
    db_session.commit()
    ...
    for card_data in data:
        assert (
            card_data["id"] != card_data["card_id"]
        ), f"id y card_id no deben ser iguales: {card_data}"
        assert "match_id" in card_data, "Falta campo match_id"
        assert card_data["match_id"] == str(
            match_id
        ), f"match_id incorrecto: {card_data['match_id']}"
