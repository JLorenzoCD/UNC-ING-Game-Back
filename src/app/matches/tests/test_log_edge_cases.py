...
class TestLogEdgeCases:
    ...
    def test_create_log_with_unicode_characters(self, db_session):
        ...
        event_type = MatchEventType.PLAYER_JOIN
        log_id = msg_service.create_log(match_id, unicode_message, event_type)
    ...
    def test_create_log_with_very_long_uuid_string(self, db_session):
        ...
        event_type = MatchEventType.PLAYER_JOIN
        with pytest.raises(Exception):
            msg_service.create_log(invalid_match_id, message, event_type)
    ...
    def test_get_log_by_id_nonexistent_log(self, db_session):
        ...
        try:
            result = msg_service.get_log_by_id(nonexistent_id)
    ...
    def test_get_msgs_by_match_with_corrupted_data(self, db_session):
        ...
        assert isinstance(logs[0], MatchMessageOut)
    ...
    def test_msg_service_memory_usage(self, db_session):
        ...
        for cycle in range(10):
            ...
            for log in logs[-5:]:
                retrieved = msg_service.get_log_by_id(log.id)
                if retrieved:
                    assert retrieved.id == log.id
        final_logs = msg_service.get_msgs_by_match(match_id)
    ...
    def test_msg_service_with_malformed_event_type(self, db_session):
        ...
        message = "Test message"
        try:
            msg_service.create_log(match_id, message, "INVALID_EVENT_TYPE")
