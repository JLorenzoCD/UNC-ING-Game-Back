...
class TestDiscardCardsEndpoint:
    ...
    def test_discard_cards_not_owned_by_player(self, client, db_session):
        ...
        with patch("app.matches.endpoints.manager.specificBroadcast", new_callable=AsyncMock), patch("app.matches.endpoints.LogServices.create_and_propagate_log", new_callable=Mock):
            response = client.put(
                f"/matches/{setup_data['match_str_id']}/cards/discard",
                json=request_data,
            )
...
class TestCombinedTakeDiscardScenarios:
    ...
    def test_take_and_discard_workflow(self, client, db_session):
        ...
        with patch("app.matches.endpoints.manager", new_callable=AsyncMock), patch("app.matches.endpoints.LogServices.create_and_propagate_log", new_callable=Mock):
            discard_data = {
                "player_id": setup_data["owner_str_id"],
                "card_ids": [
