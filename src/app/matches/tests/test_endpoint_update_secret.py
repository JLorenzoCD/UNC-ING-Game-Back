...
@pytest.mark.parametrize(
    ...
    [(Secret_action.REVEAL, True), (Secret_action.HIDE, False)],
)
def test__hide_reveal_secret(db_session, client, action, expected_state):
    """Test  hide reveal secret.

    Args:
        db_session: Parameter db_session.
        client: Parameter client.
        action: Parameter action.
        expected_state: Parameter expected_state."""
    with patch("app.matches.endpoints.manager") as mock_manager:
        ...
        set_data = {"target_player_id": player2_id, "action": action}
        response = client.put(
...
@pytest.mark.parametrize(
    "action1, expected_state, action2",
...
def test__multi_action_secret(db_session, client, action1, expected_state, action2):
    """Test  multi action secret.

    Args:
        db_session: Parameter db_session.
        client: Parameter client.
        action1: Parameter action1.
        expected_state: Parameter expected_state.
        action2: Parameter action2."""
    with patch("app.matches.endpoints.manager") as mock_manager:
        mock_manager.specificBroadcast = AsyncMock()
        mock_manager.waiting_room_broadcast = AsyncMock()
        setup_data = setup_match_and_players(client, db_session)
        ...
        target_secret_id: UUID = match_secret_ids[1]
        str_secret_id = str(target_secret_id)
        set_data = {"target_player_id": player2_id, "action": action1}
        response = client.put(
            f"/matches/{match_str_id}/secrets/{str_secret_id}",
            json=jsonable_encoder(set_data),
        )
        ...
        set_response = response.json()
        assert set_response["is_revealed"] == expected_state
        set_data = {"target_player_id": player2_id, "action": action2}
        response = client.put(
            f"/matches/{match_str_id}/secrets/{str_secret_id}",
            json=jsonable_encoder(set_data),
        )
...
def test_hide_reveal_secret_invalid(db_session, client, action, expected_state):
    """Test hide reveal secret invalid.
...
def test_steal_secret_invalid_players(db_session, client):
    """Test steal secret invalid players.
