...
def test_get_sets_with_single_set(client, db_session):
    ...
    # Crear un set en la base de datos
    match_set = Match_Set(
...
def test_get_sets_with_multiple_sets(client, db_session):
    ...
    # Crear múltiples sets en la base de datos
    sets_data = [
        {
            'type': SetType.HERCULE_POIROT,
            'player_id': owner_id,
    ...
    # Verificar que todos los sets están presentes
    ...
    # Verificar algunos detalles específicos
...
def test_get_sets_different_matches(client, db_session):
    ...
    # Obtener sets de la primera partida
    ...
    # Obtener sets de la segunda partida
...
def test_get_sets_nonexistent_match(client, db_session):
    ...
    # Debería devolver una lista vacía, no un error 404
    # según la implementación actual del endpoint
    assert response.status_code == status.HTTP_200_OK
    sets = response.json()
    assert isinstance(sets, list)
    assert len(sets) == 0
...
def test_get_sets_invalid_match_id_format(client, db_session):
    ...
    # FastAPI debería devolver 422 por validation error del UUID
...
def test_get_sets_database_error(client, db_session):
    ...
    # El endpoint maneja errores de DB devolviendo 500 según la implementación actual
    # pero en una sesión cerrada con TestClient puede comportarse diferente
    # Este test verifica que el endpoint no falle catastróficamente
    response = client.get(f"/matches/{match_id}/sets")
    
    # Verificar que la respuesta es válida (200 o 500)
...
def test_get_sets_response_format(client, db_session):
    ...
    # Crear un set con todos los campos
    match_set = Match_Set(
