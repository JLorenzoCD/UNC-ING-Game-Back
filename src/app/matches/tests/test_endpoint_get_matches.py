...
def test_get_matches(client, db_session):
    ...
    # Probamos método GET para obtener todas las partidas
    ...
    for match in data:
        ...
        assert match['current_player_count'] == 2, (
            f"la cantidad de jugadores en {match['name']}: {match['current_player_count']}"
        )

    # Probamos método GET para obtener una partida
    for i in range(5):
        response = client.get(f"/matches/{created_matches[i]['match_id']}")
        assert response.status_code == 200
        data = response.json()
        assert data['id'] == str(created_matches[i]['match_id']), "ID de partida no coincide"
        assert data['current_player_count'] == 2, (
