...
class TestMessageServices:
    ...
    def test_create_log_all_event_types(self, db_session):
        ...
        saved_logs = msg_service.get_msgs_by_match(match_id)
        ...
        with patch.object(db_session, "add", side_effect=SQLAlchemyError("DB Error")):
            with patch.object(db_session, "rollback") as mock_rollback:
                with pytest.raises(SQLAlchemyError):
                    msg_service.create_log(match_id, message, event_type)
    ...
    def test_create_log_success(self, db_session):
        ...
        saved_log = db_session.query(MatchMessage).filter(
    ...
    def test_create_log_with_empty_message(self, db_session):
        ...
        log_id = msg_service.create_log(match_id, empty_message, event_type)
        assert log_id is not None
        saved_log = db_session.query(MatchMessage).filter(
            MatchMessage.id == log_id).first()
    ...
    def test_create_log_with_long_message(self, db_session):
        ...
        long_message = "x" * 1000
        event_type = MatchEventType.PLAYER_JOIN
        log_id = msg_service.create_log(match_id, long_message, event_type)
        assert log_id is not None
        saved_log = db_session.query(MatchMessage).filter(
            MatchMessage.id == log_id).first()
    ...
    def test_create_log_without_player_id(self, db_session):
        ...
        event_type = MatchEventType.TURN
        log_id = msg_service.create_log(match_id, message, event_type)
        assert log_id is not None
        saved_log = db_session.query(MatchMessage).filter(
            MatchMessage.id == log_id).first()
        assert saved_log is not None
    ...
    def test_get_log_by_id_database_error(self, db_session):
        ...
        with patch.object(db_session, "query", side_effect=SQLAlchemyError("DB Error")):
            with patch.object(db_session, "rollback") as mock_rollback:
                with pytest.raises(SQLAlchemyError):
                    msg_service.get_log_by_id(log_id)
                mock_rollback.assert_called_once()
    ...
    def test_get_log_by_id_success(self, db_session):
        ...
        log_id = msg_service.create_log(
            match_id, message, event_type, player_id)
        retrieved_log = msg_service.get_log_by_id(log_id)
        assert isinstance(retrieved_log, MatchMessageOut)
    ...
    def test_get_msgs_by_match_empty_result(self, db_session):
        ...
        msg_service = MessageServices(db_session)
        match_id = uuid.uuid4()
        logs = msg_service.get_msgs_by_match(match_id)
        assert logs == []
    ...
    def test_msg_service_integration_with_different_matches(self, db_session):
        ...
        player_id = uuid.uuid4()
        log1_match1 = msg_service.create_log(
            match1_id, "Match 1 Log 1", MatchEventType.PLAYER_JOIN, player_id
        )
        log2_match1 = msg_service.create_log(
            match1_id, "Match 1 Log 2", MatchEventType.TURN
        )
        log1_match2 = msg_service.create_log(
            match2_id, "Match 2 Log 1", MatchEventType.HERCULE_POIROT
        )
        match1_logs = msg_service.get_msgs_by_match(match1_id)
        match2_logs = msg_service.get_msgs_by_match(match2_id)
