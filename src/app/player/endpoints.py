...
@player_router.post(path="/players", status_code=status.HTTP_201_CREATED)
async def create_player(player_info: Player_Schema_in, db=Depends(get_db)) -> Player_Schema_out:
    new_player = Player(name=player_info.name,avatar=player_info.avatar,birthday=player_info.birthday)
    db.add(new_player)
    db.commit()
