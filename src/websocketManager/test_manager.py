...
async def test_close_match_moves_all_to_waiting_room(
    ...
    await manager.enterMatch(ws1, match_id)
    await manager.enterMatch(ws2, match_id)
...
@pytest.mark.asyncio
@patch("websocketManager.ws_routes.WsEventsService")
async def test_connect_and_disconnect(mock_ws_events_service):
...
@pytest.mark.asyncio
@patch("websocketManager.ws_routes.WsEventsService")
async def test_enter_and_quit_match(mock_ws_events_service):
...
async def test_last_event_message_arrives_on_reconnection(
    mock_ws_events_service, mock_session_local
):
    """
...
@pytest.mark.asyncio
@patch("websocketManager.ws_routes.WsEventsService")
async def test_specific_broadcast(mock_ws_events_service):
    ...
    ws1.player_id = pl_id1
    ws2.player_id = pl_id2
...
@pytest.mark.asyncio
@patch("websocketManager.ws_routes.WsEventsService")
async def test_waiting_room_broadcast(mock_ws_events_service):
